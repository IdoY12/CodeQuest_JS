/**
 * Shared daily puzzle bank and client-side validation helpers.
 *
 * Responsibility: single source for seed data and mobile offline puzzle selection.
 * Layer: @project/daily-puzzles
 * Depends on: none
 * Consumers: backend prisma seed, mobile DailyPuzzleScreen
 */
...
export function pickDailyPuzzleIndex(totalPuzzles: number, date: Date): number {
  ...
  return ((dayOfYear % totalPuzzles) + totalPuzzles) % totalPuzzles;
}

export function getPuzzleForDate(date: Date): DailyPuzzle {
  return puzzles[pickDailyPuzzleIndex(puzzles.length, date)];
}
