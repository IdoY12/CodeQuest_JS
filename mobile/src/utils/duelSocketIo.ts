...
export function bindDuelSocketEvents(socket: Socket) {
  ...
  socket.on("queue_rejected", (p: { reason?: string }) => logDuel("queue:rejected", { reason: p?.reason }));
  ...
  socket.on("match_found", (p) => {
    ...
    publishDuel({
      ...
      round: null, score: { me: 0, opp: 0 }, duelEnd: null, rematchStatus: null,
    });
  });
  socket.on("round_start", (p) => {
    const q = p.question ?? {};
    publishDuel({ round: { roundNumber: p.round_number, prompt: q.prompt, codeSnippet: q.code_snippet, options: q.options ?? [], correctAnswer: q.correct_answer, type: q.type ?? "MULTIPLE_CHOICE" } });
  });
  socket.on("round_result", (p) => {
    ...
    publishDuel({ score: { me: isP1 ? p.scores.player1 : p.scores.player2, opp: isP1 ? p.scores.player2 : p.scores.player1 } });
  });
